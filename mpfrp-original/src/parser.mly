...
prog_module:
  | MODULE id = ID
    PARTY parties = separated_list(COMMA, ID)
    IN innodes = separated_list(COMMA, id_party_type)
    OUT outnodes = separated_list(COMMA, id_party_type)
    defs = nonempty_list(definition)
    EOF
    { 
      let id_and_party = NodesS.elements !nodes in
        nodes := NodesS.empty;
    {
      id = id;
      party = parties;
...
definition:
  | CONST it = id_and_type_opt EQUAL e = expr
    { Const(it, e) }
  | NODE init = option(INIT LBRACKET e = expr RBRACKET { e }) itp = id_party_type_opt EQUAL e = expr
    { update (itp2ip itp); Node(itp, init, e) }
