...
let infer_defs defs env =
  ...
  let annots = List.map (fun id -> (id,M.find id env)) ids in
...
let type_periodic module_id leaders_with_periodic env =
  List.iter (fun (id, _, expr) ->
              if List.length expr <> 1 then raise (TypeError ("periodic takes one argument: " ^ id ^ " in " ^ module_id))
              else unify TInt (infer env 1 (List.hd expr)))
            leaders_with_periodic
...
let type_module program = 
  ...
  infer_defs (List.map (fun (i,_,_,e) -> (i,e)) n) in
  check_init result n;
  type_periodic program.id (List.filter (fun (_, fun_id, _) -> fun_id = "periodic") program.leader) env;
