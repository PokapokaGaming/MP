...
let gen_embedded_runtime () =
  String.concat "\n" [
    ...
    "    end.";
    "";
    "%% --- Transaction Helper: Suspend with Retry ---";
    "%% Attempts to suspend a party, retrying if already suspended by another transaction";
    "-define(SUSPEND_RETRY_DELAY, 50).";
    "-define(SUSPEND_MAX_RETRIES, 10).";
    "";
    "mpfrp_suspend_with_retry(PartyId) ->";
    "    mpfrp_suspend_with_retry(PartyId, ?DEFAULT_TIMEOUT).";
    "";
    "mpfrp_suspend_with_retry(PartyId, Timeout) ->";
    "    mpfrp_suspend_with_retry(PartyId, Timeout, ?SUSPEND_MAX_RETRIES).";
    "";
    "mpfrp_suspend_with_retry(_PartyId, _Timeout, 0) ->";
    "    {error, max_retries_exceeded};";
    "mpfrp_suspend_with_retry(PartyId, Timeout, Retries) ->";
    "    case mpfrp_suspend_party(PartyId, Timeout) of";
    "        {ok, Token} ->";
    "            {ok, Token};";
    "        {error, already_suspended} ->";
    "            timer:sleep(?SUSPEND_RETRY_DELAY),";
    "            mpfrp_suspend_with_retry(PartyId, Timeout, Retries - 1);";
    "        {error, Reason} ->";
    "            {error, Reason}";
    "    end.";
    "";
    "%% --- Transaction Helper: Execute within Transaction ---";
    "%% Automatically wraps operation in suspend/resume for atomicity";
    "with_transaction(PartyId, Fun) ->";
    "    with_transaction(PartyId, Fun, ?DEFAULT_TIMEOUT).";
    "";
    "with_transaction(PartyId, Fun, Timeout) ->";
    "    case mpfrp_suspend_with_retry(PartyId, Timeout) of";
    "        {ok, Token} ->";
    "            try";
    "                Result = Fun(),";
    "                case mpfrp_resume_party(PartyId, Token) of";
    "                    ok -> {ok, Result};";
    "                    {error, ResumeErr} -> {error, {resume_failed, ResumeErr, Result}}";
    "                end";
    "            catch";
    "                Type:Error ->";
    "                    %% Ensure resume even on error";
    "                    catch mpfrp_resume_party(PartyId, Token),";
    "                    {error, {transaction_failed, Type, Error}}";
    "            end;";
    "        {error, Reason} ->";
    "            {error, {suspend_failed, Reason}}";
    "    end.";
    "";
    "%% --- Transaction Helper: Execute within Multi-Party Transaction ---";
    "%% Suspends multiple parties atomically, executes, then resumes all";
    "with_multi_transaction(PartyIds, Fun) ->";
    "    with_multi_transaction(PartyIds, Fun, ?DEFAULT_TIMEOUT).";
    "";
    "with_multi_transaction(PartyIds, Fun, Timeout) ->";
    "    %% Phase 1: Suspend all parties (collect tokens)";
    "    case suspend_all(PartyIds, Timeout, []) of";
    "        {ok, Tokens} ->";
    "            try";
    "                Result = Fun(),";
    "                %% Phase 2: Resume all parties";
    "                resume_all(Tokens),";
    "                {ok, Result}";
    "            catch";
    "                Type:Error ->";
    "                    %% Ensure resume even on error";
    "                    catch resume_all(Tokens),";
    "                    {error, {transaction_failed, Type, Error}}";
    "            end;";
    "        {error, Reason} ->";
    "            {error, {suspend_failed, Reason}}";
    "    end.";
    "";
    "suspend_all([], _Timeout, Acc) ->";
    "    {ok, lists:reverse(Acc)};";
    "suspend_all([PartyId | Rest], Timeout, Acc) ->";
    "    case mpfrp_suspend_with_retry(PartyId, Timeout) of";
    "        {ok, Token} ->";
    "            suspend_all(Rest, Timeout, [{PartyId, Token} | Acc]);";
    "        {error, Reason} ->";
    "            %% Rollback: resume already suspended parties";
    "            catch resume_all(Acc),";
    "            {error, {failed_to_suspend, PartyId, Reason}}";
    "    end.";
    "";
    "resume_all([]) -> ok;";
    "resume_all([{PartyId, Token} | Rest]) ->";
    "    catch mpfrp_resume_party(PartyId, Token),";
    "    resume_all(Rest).";
    "";
...
let gen_run_party () =
  String.concat "\n" [
    ...
    "            %% Already suspended - reject with explicit error for proper retry control";
    "            Caller ! {error, already_suspended},";
    "            run_party_suspended(Config, Ver, Token, Timeout, NextFireTime)";
    "    after Timeout ->";
...
let gen_exports inst_prog module_map =
  ...
  exports := "-export([mpfrp_suspend_with_retry/1, mpfrp_suspend_with_retry/2])." :: !exports;
  exports := "-export([with_transaction/2, with_transaction/3, with_multi_transaction/2, with_multi_transaction/3])." :: !exports;
  ...
  exports := "-export([get_upstream_parties_from_connections/1])." :: !exports;
...
let gen_high_level_api inst_prog =
  ...
  if List.length templates = 0 then ""
    ...
    String.concat "\n" [
      ...
      "                {error, not_found} ->";
      "                    %% Determine if transaction is needed based on connections";
      "                    UpstreamParties = get_upstream_parties_from_connections(Connections),";
      "                    case UpstreamParties of";
      "                        [] ->";
      "                            %% No upstream parties - no transaction needed";
      "                            do_spawn_worker_internal(Name, Template, Params, Connections);";
      "                        [SingleParty] ->";
      "                            %% Single upstream party - use simple transaction";
      "                            with_transaction(SingleParty, fun() ->";
      "                                do_spawn_worker_internal(Name, Template, Params, Connections)";
      "                            end);";
      "                        MultipleParties ->";
      "                            %% Multiple parties - use multi-transaction";
      "                            with_multi_transaction(MultipleParties, fun() ->";
      "                                do_spawn_worker_internal(Name, Template, Params, Connections)";
      "                            end)";
      "                    end";
      "            end";
      "    end.";
      "";
      "%% Internal spawn logic (may be called within transaction)";
      "do_spawn_worker_internal(Name, Template, Params, Connections) ->";
      ...
      "    stop_party(Name).";
      "";
      "%% hot_swap/5 - Replace a worker with a new one (Blue-Green deployment)";
      "%% Strategy: Suspend → Spawn new → Stop old → Resume (atomic transaction)";
      ...
      "        {ok, _OldStatus} ->";
      "            %% Phase 2: Determine which parties need to be suspended";
      "            %% We need to suspend any party that will be affected by the topology change";
      "            UpstreamParties = get_upstream_parties_from_connections(Connections),";
      "            case UpstreamParties of";
      "                [] ->";
      "                    %% No upstream parties - execute without transaction";
      "                    do_hot_swap_internal(OldName, NewName, Template, Params, Connections);";
      "                [SingleParty] ->";
      "                    %% Single upstream party - use simple transaction";
      "                    with_transaction(SingleParty, fun() ->";
      "                        do_hot_swap_internal(OldName, NewName, Template, Params, Connections)";
      ...
      "    end.";
      "";
      "%% Internal hot_swap logic (called within transaction)";
      "do_hot_swap_internal(OldName, NewName, Template, Params, Connections) ->";
      "    %% Spawn new worker (Blue-Green: new first)";
      "    case do_spawn_worker_internal(NewName, Template, Params, Connections) of";
      "        {error, SpawnError} ->";
      "            %% New worker failed - old worker still running (safe)";
      "            {error, {new_worker_spawn_failed, SpawnError}};";
      "        {ok, NewInfo} ->";
      "            %% Stop old worker";
      "            case stop_party(OldName) of";
      "                ok ->";
      ...
      "    end.";
      "";
      "%% Helper: Extract party names from connection map";
      "get_upstream_parties_from_connections(Connections) when is_map(Connections) ->";
      "    Nodes = maps:values(Connections),";
      "    lists:usort([extract_party_from_node(N) || N <- Nodes, N =/= undefined]);";
      "get_upstream_parties_from_connections(_) ->";
      "    [].";
      "";
